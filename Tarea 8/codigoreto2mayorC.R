...
colnames(basefiltrados) <- c("16000 (c menor)", "32000 (c menor)", "64000 (c menor)", "128000 (c menor)",
                             "16000 (c mayor)", "32000 (c mayor)", "64000 (c mayor)", "128000 (c mayor)")

plot(basefiltrados[,1], type = "l", col= "red", ylim=c(min(basefiltrados), max(basefiltrados)),
     main = "Porcentaje Filtrado por Iteraci�n (C Menor)", xlab = "Iteraciones", ylab = "Porcentaje de filtraciones")
lines(basefiltrados[,2], type = "l", col= "blue")
lines(basefiltrados[,3], type = "l", col= "black")
lines(basefiltrados[,4], type = "l", col= "green")
legend("bottomright", legend=vectorn, fill= c("red", "blue", "black", "green"))

plot(basefiltrados[,5], type = "l", col= "red", ylim=c(min(basefiltrados), max(basefiltrados)),
     main = "Porcentaje Filtrado por Iteraci�n (C Mayor)", xlab = "Iteraciones", ylab = "Porcentaje de filtraciones")
lines(basefiltrados[,6], type = "l", col= "blue")
lines(basefiltrados[,7], type = "l", col= "black")
lines(basefiltrados[,8], type = "l", col= "green")
legend("bottomright", legend=vectorn, fill= c("red", "blue", "black", "green"))
